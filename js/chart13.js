const ctx13 = document.getElementById('pie_chart13').getContext('2d');

const label7 = course.map(item => item.course);
const pressure7 = course.map(item => parseFloat(item.avg_pressure_tolerance)); // ความทนต่อแรงกดดัน
const hope7 = course.map(item => parseFloat(item.avg_hope_and_support)); // ความหวังและกำลังใจ
const obstacles7 = course.map(item => parseFloat(item.avg_overcoming_obstacles)); // การต่อสู้กับอุปสรรค

const config7 = {
    type: 'bar',
    data: {
        labels: label7, // ป้ายกำกับแกน x
        datasets: [
            {
    ...
    options: {
        ...
        plugins: {
            title: {
                display: true,
                text: 'กราฟที่ 13 : คะแนนเฉลี่ยตามความคิดเห็นต่อหลักสูตรที่เรียน',
