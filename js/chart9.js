const ctx9 = document.getElementById('pie_chart9').getContext('2d');

const label3 = fam_status.map(item => item.fam_status);
const pressure3 = fam_status.map(item => parseFloat(item.avg_pressure_tolerance)); // ความทนต่อแรงกดดัน
const hope3 = fam_status.map(item => parseFloat(item.avg_hope_and_support)); // ความหวังและกำลังใจ
const obstacles3 = fam_status.map(item => parseFloat(item.avg_overcoming_obstacles)); // การต่อสู้กับอุปสรรค

const config3 = {
    type: 'bar',
    data: {
        labels: label3, // ป้ายกำกับแกน x
        datasets: [
            {
    ...
    options: {
        ...
        plugins: {
            title: {
                display: true,
                text: 'กราฟที่ 9 : คะแนนเฉลี่ยตามสถานภาพครอบครัว',
