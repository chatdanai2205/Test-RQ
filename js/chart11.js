const ctx11 = document.getElementById('pie_chart11').getContext('2d');

const label5 = fam_earn.map(item => item.fam_earn);
const pressure5 = fam_earn.map(item => parseFloat(item.avg_pressure_tolerance)); // ความทนต่อแรงกดดัน
const hope5 = fam_earn.map(item => parseFloat(item.avg_hope_and_support)); // ความหวังและกำลังใจ
const obstacles5 = fam_earn.map(item => parseFloat(item.avg_overcoming_obstacles)); // การต่อสู้กับอุปสรรค

const config5 = {
    type: 'bar',
    data: {
        labels: label5, // ป้ายกำกับแกน x
        datasets: [
            {
    ...
    options: {
        ...
        plugins: {
            title: {
                display: true,
                text: 'กราฟที่ 11 : คะแนนเฉลี่ยตามรายได้ครอบครัว (ต่อเดือน)',
